...
- sema
  ...
  - 休眠与唤醒
    - semacquire(sema)
      - 将当前goroutine休眠
    - semrelease(sema)
      - 从使用sema信号量休眠的协程队列中取出第一个协程（休眠的更久）
...
- 所以这里key可能大于0，而那些正在休眠排队的goroutine，需要被唤醒，所以Unlock这不能偷偷的返回了。

### 问题

-  v1版本中，抢锁的众多goroutine中，不是一次性抢到都会经过休眠。
- 休眠在唤醒，涉及到协程间切换，这也是有损性能的。
- 能不能在Unlock解锁后，如果发现有正在请求锁的活跃goroutine，直接把锁给它，这就少了一次休眠在唤醒的过程，有助于提升性能。
...
- 其他情况就需要去竞争了
  ...
  - 首先是构造用于一会用来替换旧state的新值
    ...
    - `if old&mutexLocked != 0`如果原本就是加过锁状态的，需要给等待者数量加1
  ...
  - 然后通过CAS，用新值替换旧值
    ...
    - 成功
      ...
      - `if old&mutexLocked == 0`。此时发现加锁状态没了，直接break。这是区别v1的主要逻辑。
        ...
        - 对于第二种情况，在v1中虽然没有等待者状态的计数，但是它们是客观存在的，且都是休眠状态的。如果按照v1的思路，当前goroutine应该进入休眠，并且唤醒一个等待者。但是v2版本的优化就是要在抢锁过程中，让活跃的goroutine的优先级高于休眠的goroutine。所以这种情况，也应该是当前goroutine直接拿到锁
...
- Fast path，先通过去掉`AddInt32`去掉锁标志，同一时刻，Unlock的协程只可能有一个，这个操作一定成功。
  ...
  - `if (new+mutexLocked)&mutexLocked == 0`用来检验是不是对未加锁的Mutex做了Unlock操作
    - 如果state本来是无锁状态，对其AddInt32操作后，在执行if条件，结果是ture。
- 还需要”交代后事“，主要是指唤醒休眠的goroutine：
  ...
  - 如果这把锁没有等待者了，无需”交代“了。有唤醒的waiter或者锁已经又被抢到了，也无需”交代了“。
    ...
    - `if old>>mutexWaiterShift == 0`表示没有等待者。继续用int8来说明（后续的位运算举例都是用int8）
      ...
      - 00000100>> 2 = 00000001，就通不过这条if，反之能通过。
...
- 在锁还被释放的情况下，加上了自旋
  ...
  - 先不考虑`awoke`的变化，临界区代码执行时间很短的情况下，spin多几次，可能就能跳过等待拿到锁。直到达到自旋限制后，才加入等待者队列。
  - 在Unlock方法“交代后事”时，对于mutexWoken状态的锁是不需要处理的。在v2版本我分析不出这种场景什么时候会出现。但是在这里起作用了。
  - v2中，锁一旦加上了mutexWoken状态后，一定是唤醒了一个已休眠的goroutine。但是自旋模式时，新来的活跃goroutine为了多一点机会，所以它需要“骗”一下执行Unlock的goroutine，“骗”的方式是将锁加上mutexWoken状态来”冒充“被唤醒的等待者。
  - 接下来的重点就是何时”行骗“，给锁加mutexWoken了。
  ...
  - 总结：
    - UnLock时，不需要”交代后事“的情况有三种
      - 没有等待者了
      - 已经被抢到锁了
      - 有活跃goroutine进来，虽然这会锁还被释放，但是它在自旋，想”骗“一下以便于插队，否则锁没被释放的情况下，它一定得被休眠。

### 问题

- 新来的goroutine不学好，靠“骗”给自己带来了优先级。如果这种现象一直出现，那被休眠的goroutine会永无出头之日，也就是进入了饥饿状态。v4引入了饥饿模式来做一个在性能和公平之间的平衡。
...
- CAS操作，成功之后，需要处理锁的状态。
  ...
  - 除去第一种情况，直接拿到了锁，跳出Lock了，去其余情况都得考虑休眠等待的事情：
    ...
    - 把自己放在休眠队列中
      ...
      - 如果不是首次，证明曾经被唤醒过不过没抢到，所以它排在队首，让自己在下一次唤醒中，优先级变高。
...
- mutexWaiters表示锁的等待者数量、mutexWoken表示锁是否有唤醒的goroutine、mutexLocked表示锁是否被持有
- 那问题就是：当需要修改某一部分值的时候，怎样去避免影响到其他部分？自然就是通过位运算。
...
- 实际操作
  ...
  - 锁是否被持有
    ...
    - 加上锁标志
      ...
      - 采用key对mutexLocked做 | 操作
        - 11111101 | 00000001 = 11111101
        - 11111100 | 00000001 = 11111101
    - 判断是否持有锁
      - 对mutexLocked做 与 操作，由于mutexLocked的最低位是1，所以只有key的最低位是1的情况下，与操作才能得到1，并且由于mutexLocked其他位都是0，所以还能清除掉表其他值。
      - 11111101 & 00000001 = 000000001 
    ...
    - 解除持有锁状态
      ...
      - 但是在Mutex的场景中，只有可能是持有锁的goroutine做解除操作，所以第二种情况不存在，直接使用-1操作就好了
  ...
  - 等待者数量
    ...
    - 增加等待者数量，就是针对倒数第三位+1
      ...
      - state + 1 << mutexWaiterShift  ---> 00000001 + 00000100 = 00000101
    - 减少等待者数量类似
