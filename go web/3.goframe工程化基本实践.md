不去评价`goframe`框架到底怎么样，反正我接触它的这一年，在github上的认可度是高了不少。`goframe2.0`的文档绝对是一个关于web服务工程化的宝藏。对每一个模块的阅读，我都感觉写满了故事：一位大佬不断踩坑并且不断爬坑的励志故事。

而且在学习文档的过程，会有一种似成相识的感觉。自己的工作中使用的是`gin`框架，为了解决我们团队自己的痛点，也做过goframe框架作者类似的功能，比如生成代码等，不过我们想的没那么全面。
