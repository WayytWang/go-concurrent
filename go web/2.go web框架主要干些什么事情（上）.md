...
`gin`框架应该是最火的go web框架了，没有之一。本文的主要内容就是分析`gin`的核心逻辑。
...
ServeMux的主要职责是保存路由，和收到请求后找到路径对应的处理函数，其实也就是路由注册和路由匹配。Engine会比它做的更加出色，接下来会从Engine的关键代码入手，看一看gin的思路。
...
它内部保存路由的结构是一个map，key是path，value是对应的处理函数，没有任何地方可以保存请求方式。
...
`engine.trees.get(method)`找到了GET请求的路由根节点后，调用`addRoute()`更新路由树
...
和支持区分请求方式一样，`engine`能支持分组也是依靠`RouterGroup`,他还实现了另一个接口`IRouter`。
...
- Handlers：是该分组对应的处理函数列表
  - 路由对应的处理函数很好理解，分组对应的处理函数在中间件部分解释
- basePath：是该分组路由的前缀
...
- Group方法的实现很简单，就是构建了一个新的`RouterGroup`,保存了路由前缀
...
所以支持分组是为了让框架使用者编码体验感更好，并不会为每个分组单独保存一个trees。



### 路由匹配

在路由注册章节，提到了路由关系都保存在路由树里面，匹配查询自然也源自路由树。

分析路由匹配之前，先交代gin对处理函数的签名定义，相较于标准库有些不同。
...
- `Context`大有用处，在后文相关部分再做分析。这里只需要知道，它至少是封装了标准库中的` http.ResponseWriter`和` *http.Request`的
...
到这里可以确认的是，通过trees，找到了请求path对应的处理函数，只是为什么处理函数是一个列表？而且执行方式也很模糊？疑问都指向了`Engine`路由注册的时候提供的中间件能力。
...
这个验证逻辑属于非业务逻辑，和真正的业务逻辑没有关系，如果把这些逻辑全部揉在业务逻辑的`HandlerFunc`中,显然是不合理的：
...
- 但是只有最后写道trees里面去的中间件函数，才叫绑定成功，这发生在下一步。

#### IRoutes.GET(string, ...HandlerFunc) IRoutes

还是以GET函数举例，GET函数传入的HandlerFunc必有一个是业务逻辑的HandlerFunc。当然它也可以传入中间件函数，不过该中间件函数只会和这个请求path绑定关系，不会影响到分组的其它请求。
...
简言之，我们就是希望这个处理函数切片，能挨个执行完`Request`阶段的中间件逻辑，然后执行业务逻辑，最后执行`Response`阶段的业务逻辑。这是经典的中间件洋葱模型

 ![](/洋葱模型.png)

做到这一点，还得依靠上文提到的，触发处理函数执行的`ctx.Next()`
...
- 前三个矩形表示三个中间件函数，它们应该也必须调用Next()方法来拉起整一条处理函数调用链
