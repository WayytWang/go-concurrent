...
写了好几年的go后端，除了把摸鱼的主业做好之外，也打算几篇博客总结一下web相关的内容。

todo：这里后面补一个大纲

首先是从`net/http`标准库的核心逻辑开始，虽然工作中极少直接上标准库来写服务，要不然头发都不够掉的。但是它的逻辑对于去理解web框架是非常有益处的，所以路从这里开始走。
...
- `serverHandler.ServeHTTP`方法的实现中，首先会判断`serverHandler.*Server.Handler`是否存在。
  ...
  - 很多go的web框架都是把`ServeMux`重写了，而启动服务和接收客户端的请求还是使用的http库。比如`gin`框架
